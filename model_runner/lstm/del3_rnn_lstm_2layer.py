  ...
  def __init__(self,params, infer=False):
    ...
    with tf.variable_scope("rnnlm"):
      for time_step in range(params['seq_length']):
        ...
        self.y_hat_softmax=tf.nn.softmax(cell_output)
        self.y_tf=tf.argmax(self.target_data[:,time_step,:], 1)
    ...
    accuracy=accuracy/float(params['seq_length']*params['batch_size'])
