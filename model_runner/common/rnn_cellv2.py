  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with vs.variable_scope(scope or type(self).__name__,
      ...
      if self._use_peepholes:
        ...
        c = (act.NHardSigmoid(f + self._forget_bias,use_noise=self._is_training) * c_prev + act.NHardSigmoid(i,use_noise=self._is_training) *
             act.NHardTanh(j,use_noise=self._is_training))
      ...
      if self._use_peepholes:
        m = act.NHardSigmoid(o + w_o_diag * c,use_noise=self._is_training) * act.NHardTanh(c,use_noise=self._is_training)
      else:
        m = act.NHardSigmoid(o,use_noise=self._is_training) * act.NHardTanh(c,use_noise=self._is_training)
