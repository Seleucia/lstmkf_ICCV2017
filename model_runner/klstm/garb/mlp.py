...
# Set model weights
W = tf.Variable(rng.randn(), name="weight",dtype=tf.float32)
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        for (x, y) in zip(train_X, train_Y):
            _=sess.run([optimizer], feed_dict={X: x, Y: y})
            # jak= tf.test.compute_gradient(X, [1], pred, [1], x_init_value=None, delta=0.001, init_targets=None)
            print (jak)
