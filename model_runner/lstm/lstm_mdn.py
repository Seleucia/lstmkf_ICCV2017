...
class Model():
  ...
  def __init__(self,params, infer=False):
    ...
    with tf.variable_scope('rnnlm'):
      ...
      output_w1l2 = tf.get_variable("output_w1l2", [rnn_size, rnn_size],initializer=tf.contrib.layers.xavier_initializer() )
      output_b1l2 = tf.get_variable("output_bl21", [rnn_size])
      output_w2l2 = tf.get_variable("output_w2l2", [rnn_size, params['n_output']],initializer=tf.contrib.layers.xavier_initializer() )
      output_b2l2 = tf.get_variable("output_b321", [params['n_output']])

    output = tf.add(tf.matmul(rnn_output, output_w1),output_b1)
    final_output = tf.add(tf.matmul(output, output_w2),output_b2)

    outputl2 = tf.add(tf.matmul(rnn_output, output_w1l2),output_b1l2)
    final_outputl2 = tf.add(tf.matmul(outputl2, output_w2l2),output_b2l2)
    ...
    self.final_outputl2=tf.gather(final_outputl2,tf.squeeze(indices,[1]))
    self.y=tf.gather(y,tf.squeeze(indices,[1]))
    tmp = self.final_outputl2 -  self.y
    l2_loss=  tf.div(tf.nn.l2_loss(tmp), tf.cast(tf.shape(self.y)[0],tf.float32))
    ...
    self.l2loss=l2_loss
    self.loss=loss
    self.cost = loss+100*l2_loss+l2_reg
