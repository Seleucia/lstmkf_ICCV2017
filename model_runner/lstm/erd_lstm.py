...
class Model():
  def __init__(self,params, is_training=True):
    ...
    with tf.variable_scope('rnnlm'):
      output_pre_w1 = tf.get_variable("output_pre_w1", [params['n_input'], 512],initializer=tf.contrib.layers.xavier_initializer() )
      output_pre_b1 = tf.get_variable("output_pre_b1", [512])

      output_pre_w2 = tf.get_variable("output_pre_w2", [512, rnn_size],initializer=tf.contrib.layers.xavier_initializer() )
      output_pre_b2 = tf.get_variable("output_pre_b2", [rnn_size])
    ...
    flat_input=tf.reshape(self.input_data,[-1,params["n_input"]])
    flat_input = tf.nn.relu(tf.add(tf.matmul(flat_input, output_pre_w1),output_pre_b1))
    flat_input = tf.nn.relu(tf.add(tf.matmul(flat_input, output_pre_w2),output_pre_b2))
    flat_input=tf.reshape(flat_input,[params["batch_size"],params['seq_length'], params['n_input']])
